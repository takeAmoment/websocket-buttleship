...
export const checkMessageType = async (
  { type, data, id }: IClientRequest,
  ws: WebSocket,
) => {
  if (type === ClientMessageTypesEnum.REG) {
    const response = await requestHandler.handleRegRequest(
      { type, data, id },
      ws,
    );
    ...
    const updatedTable = await requestHandler.getUpdatedWinnersTable(
      index,
      name,
    );
  ...
  if (type === ClientMessageTypesEnum.ATTACK) {
    const response = await requestHandler.makeAShoot(
      data as unknown as IAttackData,
    );
    return [response];
  }
  ...
  if (type === ClientMessageTypesEnum.SINGLE_PLAY) {
    const response = await requestHandler.makeASinglePlay();
    return [response];
  }
};
